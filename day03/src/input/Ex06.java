...
public class Ex06 {
	public static void main(String[] args) {
		...
		// sc.nextInt() 	정수를 입력받는다. 아스키코드 48 ~ 57범위의 연속된 값만 가져온다
		...
		// next()는 띄어쓰기를 포함한 문장을 입력받을 수 없다
		...
		String s2;
		s2 = sc.nextLine();						// " 수영구 광안동" --> 앞에 띄어쓰기까지 같이 출력
		...
		operator = sc.next().charAt(0);	// 엔터 앞에 글자가 없어서 추출할 수 없는 예외 발생
		...
		sc.nextLine();	// 이전 식에서 버퍼에 남아있던 개행문자를 없애줌
		...
		System.out.print("연산자 입력 (+,-,*,/) : ");
		operator = sc.nextLine().charAt(0);	// 엔터 앞에 글자가 없어서 추출할 수 없는 예외 발생
