...
public class Quiz1_t {
	public static void main(String[] args) {
		...
		if(boardCount%10 != 0) {			// 나누었을때 나머지가 존재한다면
			pageCount +=1;					// 나머지 게시글도 보여주기 위한 페이지를 1추가
		}
		
		int page = ran.nextInt(pageCount) + 1;	// 페이지 개수 범위 안에서 특정 페이지 요청
		int begin = 0, end = 0;	
		...
		if (page != pageCount)	end = (page * 10);	// 마지막 페이지가 아닐 경우
		else 					end = boardCount;	// 마지막 페이지일 경우
