...
public class Ex04 {
	...
	// 원주율은 3.14로 간주
	static double getCircleArea(int radius) {		// 반지름이 정수인 경우의 함수
		double area = 0;
		area = radius * radius *3.14;
		return area;
	}
	static double getCircleArea(double radius) {	// 반지름이 실수인 경우의 함수
		double area = 0;
		area = radius * radius *3.14;
		return area;
	}
	...
	public static void main(String[] args) {
		...
		// 같은 이름으로 여러 함수가 중복정의될 수 있다
		
		// 예시1) 문자열의 substring 함수
		// 인자 1개만 전달하면 beginIndex로 취금하여 지정한 위치부터 끝까지 잘라낸다
		// 인자 2개 전달하면 beginIndex와 endIndex로 취급하여 begin부터 end이전까지 잘라낸다
...
//	static double add(int n1, int n2) {	// 첫번째 add와 반환형은 다르지만
