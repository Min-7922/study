...
public class Ex04 {
	public static void main(String[] args) {
		...
		int money_income = 0;		// 입력받은 돈을 저장할 변수
		...
		LOOP : while(true) {
			...
			case 2:	// 상품 목록을 보여주고, 선택받아서 금액을 계산하고 수량을 감소시키고 출력
				...
				if(menu == 1 && cokeAmount > 0 && money_income >= cokePrice) {
					...
					if (result.startsWith("[") == false){	// 문제없음
...
//					ionAmount -= 1;
//					money_remainder = money_income - ionPrice;
//					money_income = 0;
//					System.out.println("이온 음료 나왔습니다");
//					System.out.printf("거스름돈 : %,d원\n", money_remainder);
				}
	...
	static String vending(int menu, int money, int amount) {
		...
		if(amount < 1) {
			return "[수량이 부족합니다]";		// 문제가 있어서 처리할 수 없을 때에는 반환문자열이 [로 시작한다
		}
		if(money < price) {
			return "[금액이 부족합니다]";
		}
		System.out.printf("%s 음료 나왔습니다",name);	// 음료를 내어준다
		System.out.printf("거스름돈 : %,d원\n", money-price);
