...
public class Ex01 {
	public static void main(String[] args) {
		...
		// 다음 3개의 데이터를 각 2배로 만든 다음 화면에 출력해보세요
		int n1 = 3, n2 = 5, n3 = 8;
		// -> ni이런식으로 반복문을 돌릴 수 없다.(컴파일 이후 컴퓨터는 변수인식X)
		
		// <배열>
		// "같은 자료형"의 여러 변수를 연속된 메모리 공간에 배치하기 위해 사용하는 문법
		// 같은 데이터를 연속된 메모리 공간에 배치하고, 순번(index)로 구분하는 자료구죠
		// 이때 인덱스는 항상 0부터 시작한다
		...
		for(int i=0; i<arr.length; i++) {
			...
			// 배열 내부의 각 멤보는 각각 서로 다른 변수로 취급하면 된다
