...
public class Ex03 {
	public static void main(String[] args) {
		...
		Child3 ob = new Child3();	// 1) 서브클래스의 생성자가 호출
		
		// Child3 ob = new Child를 생성하는 순간
		// 1.부모+자식 크기 만큼의 메모리 생성
		// 2.부모에서 필드를 생성(n1)
		// 3.자식에서 필드를 생성(n2)
		// 4.생성된 객체 ob는 위 두 필드를 가리키게 된다.
		// (부모 필드를 먼저 생성하기 때문에 서브클래스에서 생성자를 만들 때 슈퍼클래스의 생성자(super())를 먼저 적어줘야한다) 
