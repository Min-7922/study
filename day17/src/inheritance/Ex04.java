...
class Child4 extends Parent4 {
	...
	// 기존 기존(함수)의 형식은 그대로 정의하고, 내용만 새로 작성하여 사용할 수 있다
	// 기존에 정의된 함수를 재정의한다 라고 하여 override라고 한다
	// 메서드 오버라이딩은 오버로딩과 달리 반환형, 함수이름, 매개변수의 형태 및 개수가 모두 일치해야 한다.
	
	// Cannot reduce the visibility of the inherited method from Parent4
	// 상속받은 메서드의 접근제한자 범위를 줄일 수 없다 (늘리는 건 가능)
	// ( 접근제한자 순서 : private -> package(기본) -> protected -> public )
	
	@Override 		// 오버라이딩 규칙에 맞지 않으면 경고를 보내고, 실행시키지 않는다
	void show() {	// 이 메서드는 상속받은 후 재정의한 메서드 입니다 라고 표시해 주는 것	
		System.out.printf("Child) db : %.2f\n", db);
	}
	
//	@Override
//	void test() {	// Cannot override the final method from Parent4
//		System.out.println("오버라이딩 가능");
//	}
	// final 클래스는 상속받을 수 없다 (예 : Scanner)
	// final 메서드는 오버라이딩 할 수 없다 (함수 내용을 바꿀 수 없다)
	// final 필드는 값을 바꿀 수 없다
