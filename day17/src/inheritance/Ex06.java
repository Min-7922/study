...
class Student extends Human {
	...
	int score;
	// 생성자에게 이름과 점수를 전달받고, 이름은 슈퍼클래스의 생성자로 보내고, 점수는 필드에 저장하세요
	public Student(String name, int score) {
		super(name);
		this.score = score;
	}
	// show()를 오버라이딩하여, 이름과 함께 점수를 함께 출력하도록 수정하세요
	@Override
	public void show() {
		System.out.printf("%s님의 점수 : %d", name,score);
	}
}
...
public class Ex06 {
	public static void main(String[] args) {
		...
		// 상속을 사용하는 경우
		// (A클래스와 B클래스의 관계 = is 관계)가 성립하면 상속을 사용한다.
		...
		// [일반 DB접속 클래스]를 이용하여 DB에 접속한다 -> [오라클 DB접속 클래스]를 이용하여 DB에 접속한다
