...
public class Ex01 {
	public static void main(String[] args) {
		...
		// 리스트 킻 컬렉션은 대부분 toString()이 오버라이딩 되어있어서 바로 출력해도 내용을 확인할 수 있다
		System.out.println("list : " + list);
		System.out.println("list.size() : "+list.size());
		System.out.println();
		// list는 배열에 자동으로 길이를 늘려주는 코드가 클래스로 구현되어 있는 것과 같다
		
		
		// list.add(int index, E e) 함수는 지정한 인덱스에 값을 추가한다
		// 덮어씌우는 것이 아닌 중간에 추가되고 뒤의 값은 한칸씩 밀려난다
		list.add(1, 30);
		System.out.println("list : " + list);
		System.out.println("list.size() : "+list.size());
		System.out.println();
		...
		// 해당 인덱스는 null값이 아닌 뒤의 값들이 한칸씩 앞으로 오게된다
		list.remove(0);	// 0번째 값 제거
		System.out.println("list : " + list);
		System.out.println("list.size() : "+list.size());
		System.out.println();
		
		// 값 30을 제거하고 싶을때
//		list.remove(30)						// 정수를 그냥 전달하면 index로 간주하여 오류 발생
		...
		System.out.println("list.get(0) : " + list.get(0));	// 값을 가져온다!
		System.out.println("list.get(1) : " + list.get(1));
		System.out.println("list.get(2) : " + list.get(2));
		System.out.println();
		
		// list.add 함수는 값을 추가하는데 사용하지만
		// list.set(index, E e) 함수는 지정한 위치의 값을 바꿀 때 사용한다
		 list.set(0,60);	// 0번째 값을 60으로 바꾸어라
		 System.out.println(list);
		 System.out.println();
		...
		// 정렬방법 2번
		 ...
		 // 리스트 내부에 특정 값이 포함되어 있는지 판별하는 함수도 있다
